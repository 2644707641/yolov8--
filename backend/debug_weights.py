...
def main():
    ...
    if not SUPABASE_URL or not SUPABASE_KEY:
        ...
        print("   请检查 .env 和 backend/.env 文件")
        return
    ...
    print("📊 查询所有权重记录...")
    try:
        response = supabase.table("model_weights").select("*").order("created_at", desc=True).execute()
        ...
        for user_id, user_weight_list in user_weights.items():
            ...
            if len(active_weights) > 1:
                print(f"  ⚠️  警告: 该用户有多个活跃权重（应该只有一个）")
    ...
    try:
        ...
        if not model_weights_bucket:
            ...
            print("   请按照 MODEL_STORAGE_SETUP.md 创建存储桶")
        else:
                    ...
                    for file in files:
                        ...
                        try:
                            ...
                            for subfile in subfiles:
                                subfile_name = subfile.get('name') if isinstance(subfile, dict) else subfile.name
