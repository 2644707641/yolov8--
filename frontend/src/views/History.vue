    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="card">
        ...
        <div class="mb-6 flex items-center gap-3">
          ...
          >
            图片 ({{
              detectionStore.detectionHistory.filter(
                (h) => h.file_type === "image",
              ).length
            }})
          </button>
        ...
        <div class="flex justify-between items-center mb-6">
          <div class="flex items-center space-x-4">
            <h2 class="text-lg font-semibold">
              当前显示 {{ paginatedHistory.length }} 条 / 共
        ...
        >
          <div class="flex flex-col items-center gap-3">
            <div
              class="h-12 w-12 animate-spin rounded-full border-4 border-primary-500/20 border-t-primary-400"
            ></div>
        ...
        <div
          ...
          class="text-center py-12 text-slate-500/70"
        >
          <svg
        ...
        <div v-else class="space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            >
              ...
              <!-- 原始图片和结果图片对比 -->
              <div
                class="grid grid-cols-2 gap-1 rounded-xl border border-white/10 bg-white/5 p-2 backdrop-blur-sm"
              >
                <!-- 原始图片 -->
                <div
                  class="relative group cursor-pointer"
                  @click="openPreview(item, 'original')"
                >
                  <img
                  ...
                  ></video>
                  <div
                    class="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-30 transition-all rounded flex items-center justify-center"
                  >
                    <span
                  ...
                  <div class="absolute bottom-1 left-1">
                    <span
                      class="px-2 py-0.5 text-xs font-medium rounded bg-slate-900/70 text-white"
                    >
                      原始
                    </span>
                  </div>
                ...
                >
                  ...
                  <div class="absolute bottom-1 left-1">
                    <span
                      class="px-2 py-0.5 text-xs font-medium rounded bg-primary-500/100 text-white"
                    >
                      结果
                    </span>
                  </div>
              ...
              <div class="p-4">
                ...
                <div
                  class="grid grid-cols-2 gap-2 text-xs text-slate-300/80 mb-3"
                >
                  <div>
                    <span class="font-medium">置信度:</span>
                    {{ item.params?.confidence || "N/A" }}
                  </div>
                  <div>
                    <span class="font-medium">尺寸:</span>
                    {{ item.params?.imgSize || "N/A" }}px
                  </div>
                </div>
      ...
      <Teleport to="body">
        ...
        >
          <div class="absolute top-6 right-8 z-[80] flex items-center gap-4">
            ...
            <div
              v-if="
                previewItem?.file_type === 'video' && previewMode === 'both'
              "
          ...
          >
            ...
            >
              <div
                class="flex w-full shrink-0 items-center justify-between text-xs uppercase tracking-[0.3em] text-slate-300/70"
              >
                <span>原始素材</span>
                <span v-if="previewItem.file_type === 'image'"
                  >拖拽平移 · 滚轮缩放</span
                >
              ...
              <div
                :class="[
                  'relative mt-4 flex min-h-0 flex-1 w-full items-center justify-center overflow-hidden rounded-2xl border border-white/15 bg-slate-900/40',
                  previewItem.file_type === 'image' ? 'cursor-move' : '',
                ]"
                @mousedown="
                  (e) => previewItem.file_type === 'image' && startDrag(e)
                "
                @mousemove="
                  (e) => previewItem.file_type === 'image' && onDrag(e)
                "
                @mouseup="previewItem.file_type === 'image' && endDrag()"
                @mouseleave="previewItem.file_type === 'image' && endDrag()"
                @wheel.prevent="
                  (e) => previewItem.file_type === 'image' && onWheel(e)
                "
              >
                <img
...
const filteredHistory = computed(() => {
  ...
  return detectionStore.detectionHistory.filter(
    (item) => item.file_type === filterType.value,
  );
});
...
const historyError = computed(
  () => detectionStore.historyError || detectionStore.requestError,
);
