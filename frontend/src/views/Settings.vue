  ...
  <div class="space-y-8">
    ...
    >
      <div class="rounded-2xl border border-white/10 bg-slate-950/35 px-4 py-3">
        <p class="text-[11px] uppercase tracking-[0.28em] text-slate-400/70">账号</p>
        <p class="mt-2 truncate text-sm font-medium text-white">{{ userEmail }}</p>
      </div>
      <div class="rounded-2xl border border-white/10 bg-slate-950/35 px-4 py-3">
        <p class="text-[11px] uppercase tracking-[0.28em] text-slate-400/70">系统版本</p>
        <p class="mt-2 text-sm font-medium text-white">v{{ systemInfo.apiVersion }}</p>
      </div>
      <div class="rounded-2xl border border-white/10 bg-slate-950/35 px-4 py-3">
        <p class="text-[11px] uppercase tracking-[0.28em] text-slate-400/70">默认模型</p>
        <p class="mt-2 truncate text-sm font-medium text-white">{{ systemInfo.defaultModelName }}</p>
      </div>
      <div class="rounded-2xl border border-white/10 bg-slate-950/35 px-4 py-3">
        <p class="text-[11px] uppercase tracking-[0.28em] text-slate-400/70">同步范围</p>
      ...
      <div data-testid="settings-section-defaults" class="card">
        <div class="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p class="mt-2 text-sm text-slate-300/80">用于新任务的默认检测配置，优先保证稳定性和资源利用率。</p>
            </div>
            <span class="text-xs text-slate-400">应用于新任务</span>
          </div>
          <div class="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-[repeat(2,minmax(0,1fr))_220px]">
            ...
            <label class="rounded-2xl border border-white/10 bg-white/5 p-5 xl:row-span-2">
      ...
      <div data-testid="settings-section-storage" class="card">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p class="mt-2 text-sm text-slate-300/80">集中管理本地留存、备份和清理策略，避免历史数据持续膨胀。</p>
          </div>
          <button
        ...
        <div class="mt-6 grid gap-4 xl:grid-cols-[minmax(0,1.2fr)_minmax(320px,0.8fr)]">
          <div data-testid="settings-storage-config-card" class="rounded-2xl border border-white/10 bg-white/5 p-5">
            <div class="grid gap-3 sm:grid-cols-2">
              <div class="rounded-2xl border border-white/10 bg-slate-950/30 p-4">
                <p class="text-xs uppercase tracking-[0.28em] text-slate-400/70">存储模式</p>
                <p class="mt-3 text-sm font-medium text-white">{{ storageModeLabel }}</p>
              </div>
              <div class="rounded-2xl border border-white/10 bg-slate-950/30 p-4">
                <p class="text-xs uppercase tracking-[0.28em] text-slate-400/70">历史保留</p>
                <p class="mt-3 text-sm font-medium text-white">{{ storagePolicy.retentionDays }} 天</p>
              </div>
              <div class="rounded-2xl border border-white/10 bg-slate-950/30 p-4">
                <p class="text-xs uppercase tracking-[0.28em] text-slate-400/70">备份频率</p>
                <p class="mt-3 text-sm font-medium text-white">每天 {{ storagePolicy.backupTime }}</p>
              </div>
              <div class="rounded-2xl border border-white/10 bg-slate-950/30 p-4">
                <p class="text-xs uppercase tracking-[0.28em] text-slate-400/70">存储区域</p>
                <p class="mt-3 text-sm font-medium text-white">{{ storagePolicy.region }}</p>
              </div>
            </div>

            <div class="mt-4 rounded-2xl border border-white/10 bg-slate-950/30 p-4">
              <p class="text-xs uppercase tracking-[0.28em] text-slate-400/70">默认模型</p>
              <p class="mt-3 text-sm font-medium text-white">{{ systemInfo.defaultModelName }}</p>
            </div>
          ...
          <div data-testid="settings-storage-status-card" class="rounded-2xl border border-white/10 bg-white/5 p-5">
      ...
      <div data-testid="settings-section-realtime" class="card">
          <div class="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p class="mt-2 text-sm text-slate-300/80">这里的选项只影响实时监控页面，保存后会写入本地浏览器。</p>
            </div>
          ...
          <div class="mt-6 grid gap-4 md:grid-cols-3">
            <div class="rounded-2xl border border-white/10 bg-white/5 p-5">
              ...
              <label class="mt-4 flex items-center justify-between text-sm text-slate-300/80">
                默认开启录制
      ...
      <div data-testid="settings-section-system" class="card">
          <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p class="mt-2 text-sm text-slate-300/80">这一部分主要用于查看系统边界和同步规则，不建议频繁改动。</p>
            </div>
            <span class="text-xs text-slate-400">只读信息</span>
          </div>
          <div class="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <div class="rounded-2xl border border-white/10 bg-white/5 p-4">
              <p class="text-xs uppercase tracking-[0.28em] text-slate-400/70">最大上传</p>
              <p class="mt-3 text-sm font-medium text-white">{{ systemInfo.maxUploadSizeMb }} MB</p>
            </div>
            <div class="rounded-2xl border border-white/10 bg-white/5 p-4">
              <p class="text-xs uppercase tracking-[0.28em] text-slate-400/70">最大并发</p>
              <p class="mt-3 text-sm font-medium text-white">{{ systemInfo.maxConcurrentDetections ?? '未暴露' }}</p>
            </div>
            <div class="rounded-2xl border border-white/10 bg-white/5 p-4">
              <p class="text-xs uppercase tracking-[0.28em] text-slate-400/70">系统版本</p>
              <p class="mt-3 text-sm font-medium text-white">v{{ systemInfo.apiVersion }}</p>
            </div>
            <div class="rounded-2xl border border-white/10 bg-white/5 p-4">
              <p class="text-xs uppercase tracking-[0.28em] text-slate-400/70">接口名称</p>
              <p class="mt-3 text-sm font-medium text-white">{{ systemInfo.apiTitle }}</p>
            </div>
          </div>
          <div class="mt-4 rounded-2xl border border-dashed border-white/10 bg-slate-950/30 p-4 text-sm text-slate-300/80">
    ...
    >
      <div class="mx-auto flex max-w-7xl flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div class="text-sm text-slate-300/80">
          ...
          <p class="mt-1 text-xs text-slate-400">默认推理参数与本地清理策略会同步到后端，实时识别偏好保存在当前浏览器。</p>
        </div>
        <div class="flex items-center gap-3">
          <button
            class="btn-secondary text-sm"
